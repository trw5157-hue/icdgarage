        ...
        <Card className="bg-zinc-900 border-zinc-800">
          ...
          <CardContent>
            <Tabs value={isLogin ? "login" : "register"} onValueChange={(v) => setIsLogin(v === "login")} className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-zinc-800 mb-6 p-1 rounded-lg">
                <TabsTrigger 
                  value="login" 
                  data-testid="login-tab" 
                  className="text-sm md:text-base rounded-md data-[state=active]:bg-red-600 data-[state=active]:text-white transition-all"
                >
                  Login
                </TabsTrigger>
                <TabsTrigger 
